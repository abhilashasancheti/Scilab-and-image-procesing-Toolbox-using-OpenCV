//Abhilasha Sancheti
//sample input : arrowedLine( width, height, x1 , y1 , x2,y2 )
#include <numeric>
...
extern "C"
{
  ...
  int opencv_clipline(char *fname, unsigned long fname_len)
  {
    ...
    sciErr = getMatrixOfDouble(pvApiCtx, piAddr3, &iRows, &iCols ,&x1);
    if(sciErr.iErr)
    {
        printError(&sciErr, 0);
        return 0;
    ...
    intErr = getMatrixOfDouble(pvApiCtx, piAddr6 ,&y2);
     if(intErr)
      return intErr;
    ...
    if(height<0)
    {
        sciprint("Positive Value Required for Height.");
        height=1;
        return 0;
    }
    if(width<0)
    {
        sciprint("Positive Value Required for Width.");
        width=1;
        return ;
    }  
