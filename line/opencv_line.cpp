//Abhilasha Sancheti
//sample input : line(image , x1 , y1 , x2,y2,r_value,g_value,b_value,thickness,linetype,shift)
#include <numeric>
...
extern "C"
{
  ...
  int opencv_line(char *fname, unsigned long fname_len)
  {
    ...
    intErr = getMatrixOfDouble(pvApiCtx, piAddr3 ,&y1);
     if(intErr)
      return intErr;
    ...
    intErr = getMatrixOfDouble(pvApiCtx, piAddr6 ,&r_value);
    if(intErr)
      return intErr;
    ...
    intErr = getMatrixOfDouble(pvApiCtx, piAddr9,&thickness);
    if(intErr)
      return intErr;
...
//calling the opencv function
    line( src, pt1, pt2, Scalar(r_value,g_value,b_value), thickness, linetype, shift);
