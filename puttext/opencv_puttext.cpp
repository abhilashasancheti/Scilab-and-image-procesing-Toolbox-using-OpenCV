//Abhilasha Sancheti
//sample input :puttext(image ,x1 ,y1 ,fontscale,r_value,g_value,b_value,text,fontface ,thickness,linetype,bottomleft(0/1))
#include <numeric>
...
extern "C"
{
  ...
  int opencv_puttext(char *fname, unsigned long fname_len)
  {
    ...
    sciErr = getMatrixOfDouble(pvApiCtx, piAddr5,&r_value);
     if(intErr)
      return intErr;
    ...
    sciErr = getMatrixOfDouble(pvApiCtx, piAddr6 ,&g_value);
    if(intErr)
      return intErr;
    ...
    sciErr = getMatrixOfDouble(pvApiCtx, piAddr7 ,&b_value);
    if(intErr)
      return intErr;
   ...
   //for line type of text default: 8 
    ...
    {
       bottomLeftOrigin = false;
       sciprint("Only 0 or 1 acceptable for bootomleftorigin parameter");
       return 0;  
    }
    ...
    if(strcmp(fontface,FONT_HERSHEY_SIMPLEX")==0)
           fontFace = FONT_HERSHEY_SIMPLEX;
    else if (strcmp(fontface,"FONT_HERSHEY_PLAIN")==0)
          fontFace= FONT_HERSHEY_PLAIN;
    else if (strcmp(fontface,"FONT_HERSHEY_DUPLEX")==0)
          fontFace= FONT_HERSHEY_DUPLEX;
    else if (strcmp(fontface,"FONT_HERSHEY_COMPLEX")==0)
          fontFace= FONT_HERSHEY_COMPLEX;
    else if (strcmp(fontface,"FONT_HERSHEY_TRIPLEX")==0)
          fontFace = FONT_HERSHEY_TRIPLEX;
    else if (strcmp(fontface,"FONT_HERSHEY_COMPLEX_SMALL")==0)
          fontFace = FONT_HERSHEY_COMPLEX_SMALL;
    else if (strcmp(fontface,"FONT_HERSHEY_SCRIPT_SIMPLEX")==0)
         fontFace = FONT_HERSHEY_SCRIPT_SIMPLEX;
    else if (strcmp(fontface,"FONT_HERSHEY_SCRIPT_COMPLEX")==0)
         fontFace = FONT_HERSHEY_SCRIPT_COMPLEX;
    else
       { 
            sciprint("Invalid fontface typed");
            return 0;
        }
    ...
    if( r_value <0 || r_value >255)
     {
          r_value=0;
          sciprint(" r value of colour should be between 0 and 255.");
          return 0;
     }
     if( g_value <0 || g_value >255)
     {
          g_value=0;
          sciprint("  g value of colour should be between 0 and 255.");  
          return 0;
     }
    if( b_value <0 || b_value >255)
     {
          b_value=0;
          sciprint(" b value of colour should be between 0 and 255.");
          return 0;
     }
    if ((linetype!=0) && (linetype!=4)&& (linetype!=8))
    {
             linetype=8;
             sciprint("Only 0/4/8 allowed for linetype");
             return 0;
     }
   ...
   //syntax: putText(Mat& img, const string& text, Point org, int fontFace, double fontScale, Scalar color, int thickness=1, int lineType=8, bool bottomLeftOrigin=false )
    putText( src, text[0], pt1, fontFace, fontscale, Scalar(r_value,g_value,b_value), thickness, linetype, bottomLeftOrigin );
